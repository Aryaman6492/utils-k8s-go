...
func TestExtractMetadataFromJsonBytes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			annotations: map[string]string{
				"seclogic.io/status": "",
				"seclogic.io/wlid":   "wlid://cluster-gke_armo-test-clusters_us-central1-c_danielg/namespace-seclogic/deployment-storage",
			},
			labels: map[string]string{
				"seclogic.io/workload-api-group":   "apps",
				"seclogic.io/workload-api-version": "v1",
				"seclogic.io/workload-kind":        "Deployment",
				"seclogic.io/workload-name":        "storage",
		...
		},
		{
			...
			namespace: "seclogic",
			annotations: map[string]string{
				"seclogic.io/image-id": "quay.io/seclogic/seclogic@sha256:608b85d3de51caad84a2bfe089ec2c5dbc192dbe9dc319849834bf0e678e0523",
				"seclogic.io/status":   "",
			},
			labels: map[string]string{
				"seclogic.io/image-id":   "quay-io-seclogic-seclogic-sha256-608b85d3de51caad84a2bfe089ec",
				"seclogic.io/image-name": "quay-io-seclogic-seclogic",
			},
			ownerReferences:       map[string]string{},
...
func Test_parseCalicoSelector(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "real",
			value: []byte(`"app.kubernetes.io/instance == 'seclogic' && app.kubernetes.io/name == 'operator' && tier == 'ks-control-plane'"`),
			want:  map[string]string{"app.kubernetes.io/instance": "seclogic", "app.kubernetes.io/name": "operator", "tier": "ks-control-plane"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
